...
public class SqlQueries {
    ...
    //language=HSQLDB
    String select05 = "SELECT LASTNAME, SALARY, DEP.NAME FROM EMPLOYEE EMP, DEPARTMENT DEP WHERE EMP.DEPARTMENT = DEP.ID";

    //Select all employees and corresponding department name if there is one.
    //Name column containing name of the department "depname".
    //language=HSQLDB
    String select06 = "SELECT LASTNAME, SALARY, DEP.NAME AS DEPNAME FROM EMPLOYEE EMP LEFT JOIN DEPARTMENT DEP ON DEP.ID = EMP.DEPARTMENT";
    ...
    //language=HSQLDB
    String select08 = "SELECT DEP.NAME AS DEPNAME, COUNT(EMP.DEPARTMENT) AS STAFF_SIZE FROM DEPARTMENT DEP INNER JOIN EMPLOYEE EMP ON DEP.ID = EMP.DEPARTMENT GROUP BY DEP.NAME";

    //Select all departments and values of total and average salary per department
    //Name column containing name of the department "depname".
    //language=HSQLDB
    String select09 = "SELECT DEP.NAME AS DEPNAME, SUM(SALARY) AS TOTAL, AVG(SALARY) AS AVERAGE FROM DEPARTMENT DEP INNER JOIN EMPLOYEE EMP ON DEP.ID = EMP.DEPARTMENT GROUP BY DEP.NAME";
